    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f_img i) : COLOR
            {
                ...
                noise.y += (tex2D(_MainTex, uv).a - 0.5) * 2;
                ...
                float perturb = ((1 - i.uv.y) * 0.35) + 0.02;
                noise = (noise * perturb) + i.uv - 0.02;

                fixed4 colour = tex2D(_MainTex, noise);
                colour = fixed4(colour.r * 2, colour.g * 0.9, (colour.g / colour.r) * 0.2, 1);
                noise = clamp(noise, 0.05, 1);
                colour.a = tex2D(_MainTex, noise).b * 2;
                colour.a = colour.a * tex2D(_MainTex, i.uv).b;
